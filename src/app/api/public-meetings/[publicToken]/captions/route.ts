...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const { data, error } = await admin
    ...
    .select("id, started_at, source_language, source_text, korean_text, is_final, rt_meeting_participants(display_name, company)")
  ...
  const captions = (data ?? []).map((segment) =>
    createCaptionEvent({
      ...
      translatedText: query.listeningLanguage === "ko" ? segment.korean_text : segment.source_text,
